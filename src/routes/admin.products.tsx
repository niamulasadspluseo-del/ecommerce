...
function ProductsAdmin() {
  ...
  function save() {
    ...
    admin.saveProduct(edit); setOpen(false); toast.success("Saved");
  }
      ...
      <div className="space-y-2">
        {products.map((p) => (
          <Card key={p.id} className="p-3 flex items-center gap-3">
            ...
            <Button size="icon" variant="ghost" onClick={() => { if (confirm("Delete?")) admin.deleteProduct(p.id); }}><Trash2 className="h-4 w-4" /></Button>
          </Card>
        ))}
      </div>
