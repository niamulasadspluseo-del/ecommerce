  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 space-y-3">
        ...
        <Button onClick={() => { if (!t.name || !t.text) return; admin.saveTestimonial(t); setT({ id: "", name: "", role: "", text: "", rating: 5 }); }}>Add testimonial</Button>
      </Card>
      <div className="space-y-2">
        {items.map((x) => (
          <Card key={x.id} className="p-4 flex items-start gap-3">
            ...
            <Button size="icon" variant="ghost" onClick={() => admin.deleteTestimonial(x.id)}><Trash2 className="h-4 w-4" /></Button>
          </Card>
        ))}
      </div>
    </div>
