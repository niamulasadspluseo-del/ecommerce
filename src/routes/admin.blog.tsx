      ...
      <div className="space-y-2">
        {posts.map((p) => (
          <Card key={p.id} className="p-3 flex gap-3 items-center">
            ...
            <Button size="icon" variant="ghost" onClick={() => admin.deleteBlog(p.id)}><Trash2 className="h-4 w-4" /></Button>
          </Card>
        ))}
      </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {edit && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2"><Button onClick={() => { admin.saveBlog(edit); setOpen(false); }}>Save</Button><Button variant="ghost" onClick={() => setOpen(false)}>Cancel</Button></div>
            </div>
          )}
        </DialogContent>
      </Dialog>
