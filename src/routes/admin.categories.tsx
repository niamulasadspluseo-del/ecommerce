    ...
    <div className="space-y-8">
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2 mb-4">
          {categories.map((c) => (
            <div key={c.id} className="flex items-center gap-2 border rounded-md px-3 py-2">
              ...
              <Button size="icon" variant="ghost" onClick={() => admin.deleteCategory(c.id)}><Trash2 className="h-4 w-4" /></Button>
            </div>
          ))}
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => { if (!name) return; admin.saveCategory({ id: "", name, slug: "", icon }); setName(""); setIcon(""); toast.success("Added"); }}><Plus className="h-4 w-4" /></Button>
        </div>
      </Card>
      ...
      <Card className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {tags.map((t) => <Badge key={t} variant="secondary" className="gap-1">{t}<button onClick={() => admin.saveTags(tags.filter((x) => x !== t))}>×</button></Badge>)}
        </div>
        <div className="flex gap-2">
          <Input placeholder="New tag" value={tag} onChange={(e) => setTag(e.target.value)} />
          <Button onClick={() => { if (!tag) return; admin.saveTags([...tags, tag]); setTag(""); }}><Plus className="h-4 w-4" /></Button>
        </div>
      </Card>
    </div>
  );
