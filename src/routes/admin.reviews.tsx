          ...
          return (
            <Card key={r.id} className="p-4 flex gap-3 items-start">
              ...
              <div className="flex gap-2">
                {!r.approved && <Button size="sm" onClick={() => admin.saveReview({ ...r, approved: true })}><Check className="h-4 w-4 mr-1" />Approve</Button>}
                {r.approved && <span className="text-xs px-2 py-1 bg-muted rounded">Approved</span>}
                <Button size="icon" variant="ghost" onClick={() => admin.deleteReview(r.id)}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </Card>
          );
        })}
