  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="p-6">
        ...
        <Button className="mt-4" onClick={() => { if (!c.code) return toast.error("Code required"); admin.saveCoupon(c); toast.success("Saved"); setC({ code: "", type: "percent", value: 10, usageLimit: 100, usedCount: 0 }); }}>Save coupon</Button>
      </Card>
        ...
        <div className="space-y-2">
          {coupons.map((co) => (
            <div key={co.code} className="flex items-center gap-3 border rounded-md p-3">
              ...
              <Button size="icon" variant="ghost" className="ml-auto" onClick={() => admin.deleteCoupon(co.code)}><Trash2 className="h-4 w-4" /></Button>
            </div>
          ))}
        </div>
      </Card>
