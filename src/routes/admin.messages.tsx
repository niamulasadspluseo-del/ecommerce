        ...
        <div className="grid md:grid-cols-[360px_1fr] gap-4">
          <div className="space-y-2 md:max-h-[70vh] md:overflow-y-auto pr-1">
            {messages.map((m) => (
              <Card
                key={m.id}
                onClick={() => { setSelectedId(m.id); if (!m.read) admin.markMessageRead(m.id, true); }}
              ...
              <Card className="p-6">
                <div className="flex items-start justify-between gap-3 mb-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <Button variant="outline" size="sm" onClick={() => admin.markMessageRead(selected.id, !selected.read)}>
                      <MailOpen className="h-4 w-4 mr-1" />{selected.read ? "Mark unread" : "Mark read"}
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => { admin.deleteMessage(selected.id); setSelectedId(null); }}>
