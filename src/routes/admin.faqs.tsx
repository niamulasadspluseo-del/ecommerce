  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 space-y-3">
        ...
        <Button onClick={() => { if (!f.question) return; admin.saveFaq(f); setF({ id: "", question: "", answer: "" }); }}>Add FAQ</Button>
      </Card>
      <div className="space-y-2">
        {faqs.map((x) => (
          <Card key={x.id} className="p-4 flex items-start gap-3">
            ...
            <Button size="icon" variant="ghost" onClick={() => admin.deleteFaq(x.id)}><Trash2 className="h-4 w-4" /></Button>
          </Card>
        ))}
      </div>
    </div>
