...
function Checkout() {
  ...
  const [method, setMethod] = useState<"stripe" | "crypto">(stripe.enabled ? "stripe" : "crypto");
  ...
  function placeOrder(e: React.FormEvent) {
    ...
    try {
      if (method === "stripe") {
        ...
        const o = orders.create({ method: "stripe", cardLast4: card.number.slice(-4) });
        toast.success("Payment received! Email sent (simulated).");
        ...
        const o = orders.create({ method: "crypto", txid, network: net?.name + " (" + net?.chain + ")" });
